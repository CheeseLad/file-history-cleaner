...
def list_folders_with_files_and_strings(directory_path, string_csv_path, file_csv_path, output_file='folders.json'):
...
def copy_and_rename_files(folder_info, source_root, output_root):
    ...
    for folder_id, folder in folder_info.items():
        for file_entry in folder['files']:
            ...
            rel_path_norm = rel_path.replace(':', '').replace('\\', '/').replace('\\', '/')
            ...
            try:
                ...
                bad_paths.append({'src': src_file, 'dest': dest_dir, 'reason': f'Failed to create directory: {e}'})
                continue
            new_name = file_entry['string'] if file_entry['string'] else file_entry['name']
            ...
            if os.name == 'nt' and (len(dest_file) > 255 or len(dest_dir) > 240):
                ...
                bad_paths.append({'src': src_file, 'dest': dest_file, 'reason': 'path too long'})
                continue
            # Check if source file exists
            if not os.path.exists(src_file):
                print(f"Source file does not exist: {src_file}")
                bad_paths.append({'src': src_file, 'dest': dest_file, 'reason': 'source file does not exist'})
                continue
...
def main():
    ...
    sorted_folder_info = list_folders_with_files_and_strings(of_directory, string_csv_path, filepath)
